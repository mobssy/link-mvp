...
final class ContactsSyncService: ObservableObject {
    ...
    func loadContactIdentifiers() throws -> [ContactIdentifier] {
        ...
        try store.enumerateContacts(with: request) { contact, _ in
            ...
            if !phones.isEmpty || !emails.isEmpty {
                identifiers.append(ContactIdentifier(name: fullName, phones: Array(Set(phones)), emails: Array(Set(emails))))
            }
        }
        return identifiers
    }
    ...
    func syncAndMatch() async throws -> [MatchedUser] {
        let contacts = try loadContactIdentifiers()
